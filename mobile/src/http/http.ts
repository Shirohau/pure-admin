  ...
  return new Promise<T>((resolve, reject) => {
    uni.request({
      ...
      success: async (res) => {
        ...
        // 检查是否是401错误（包括HTTP状态码401或业务码401，以及后端统一认证失效码 4001/4002）
        const isTokenExpired = res.statusCode === 401
          || code === ResultEnum.Unauthorized
          || code === ResultEnum.AuthExpired
        ...
        if (res.statusCode >= 200 && res.statusCode < 300) {
          ...
          return resolve(options.returnFullResponse ? (responseData as T) : (responseData.data as T))
        }
