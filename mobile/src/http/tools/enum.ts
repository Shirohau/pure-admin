...
export enum ResultEnum {
  ...
  Success0 = 0, // 成功
  Success200 = 200, // 成功
  // 本项目后端成功业务码（extends/drf/response.py CODE_SUCCESS=2000）
  Success2000 = 2000, // 后端统一成功码
  Error = 400, // 错误
  Unauthorized = 401, // 未授权
  // 本项目后端认证失效业务码（extends/drf/response.py）
  AuthExpired = 4001, // 未登录（CODE_UNAUTHENTICATED）
  InvalidToken = 4002, // token 无效或过期（CODE_INVALID_TOKEN）
...
export function isSuccessResultCode(code: number): boolean {
  // 兼容 0 / 200 / 2000 三种成功码（2000 为本项目后端统一成功码）
  return [ResultEnum.Success0, ResultEnum.Success200, ResultEnum.Success2000].includes(code)
}
