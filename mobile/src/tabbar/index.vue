...
.border-and-fixed {
  ...
  right: 0;
  /*
    层级说明：原先 z-index: 1000 高于 uni-modal/uni-mask 遮罩（z-index: 999），
    导致 showModal 等系统弹窗的遮罩盖不住 tabbar（退出登录确认框底部漏出导航栏）。
    现降至 99：低于所有系统浮层（modal 999 / toast / wot 弹层），高于普通页面内容，
    保证弹窗遮罩能完整覆盖整屏，tabbar 也不会被页面内容遮挡。
  */
  z-index: 99;
