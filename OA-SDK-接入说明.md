...
外接应用现在会在页面提示失败步骤、ECP 返回的 HTTP 状态、错误编号和平台请求编号，不需要从浏览器 Network 中寻找已经消失的请求。比如“准备登录失败 … HTTP 401，client_authentication_invalid”表示 ECP 拒绝了当前客户端凭据；`redirect_uri_invalid` 表示完整回调地址不一致；`https_required` 表示 ECP 自己仍未识别 HTTPS。外接应用的测试开关不改变 ECP 的平台校验。

“准备登录”发生在 SDK 取码之前；“兑换登录码”“读取身份验签公钥”“确认员工与企业身份”发生在后续不同步骤。根据页面上的具体错误编号处理对应配置；不要根据统一的 502 状态猜测是凭据错误。页面不展示平台原始响应正文、Authorization、Client Secret、登录码或令牌。
