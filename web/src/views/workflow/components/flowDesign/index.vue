<!--
  ...
  作用：流程画布容器，支持滚轮/按钮缩放、拖拽平移、视图重置；
       渲染递归节点树（node-warp）与流程结束节点；挂载审批人/抄送人/条件
       三个配置抽屉（可见性由 Pinia store 控制）；对外暴露 validatePublish
       （发布校验，收集未配置节点错误）与 getData（返回格式化后的流程数据）。
-->
<template>
...
import { FormatUtils } from '@/views/workflow/utils/workflow/formatCommitData';
import { NodeType, placeholderList } from '@/views/workflow/utils/workflow/constant.js';
...
/**
 * 将 store 中的抽屉可见性状态包装为可写 computed（供 v-model 使用）
 */
