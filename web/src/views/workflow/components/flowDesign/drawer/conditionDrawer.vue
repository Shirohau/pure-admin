<!--
  ...
  作用：编辑条件节点（分支）的规则：默认分支仅展示提示；普通分支支持多条件组
       （组间/组内"且/或"关系），按字段类型（输入/数字/日期/下拉/时间/复选/单选/
       开关）渲染对应条件编辑器；通过 selectConditionDialog 选择条件字段；
       提交后写回 store 中的 conditionsConfig（含显示名与校验错误标记）。
-->
<template>
...
/** 数字类型条件：空值归一化为 null（避免 el-input-number 边界问题） */
...
/** 验证节点属性：默认分支恒通过；普通分支每个条件组至少有一条已填写的条件 */
