<!--
  ...
  作用：从 vForm 动态表单字段中（store.lowCodeFormField）选择可用于条件分支的字段，
       支持 input/number/select/checkbox/radio/switch/date 类型；已选字段回显勾选，
       确认后通过 change 事件返回完整字段信息（含数据库类型映射与默认值）。
-->
<template>
