<!--
  ...
  作用：搜索并选择角色（左侧角色列表 + 右侧已选列表），确认后通过 change 事件
       返回 { type: 3, targetId, name } 结构的角色列表；数据来自 mock 接口。
-->
<template>
...
import { getRoleList } from "@/views/workflow/api/workflow/mock";
...
/**
 * 确认/保存：将选中角色转换为 { type: 3, targetId, name } 结构
 */
let saveDialog = () => {
    let checkedList = [
