<!--
  ...
  作用：渲染节点之间的"+"添加按钮，悬停弹出节点类型菜单
       （审批人/抄送人/条件网关/并行审批），点击后通过 NodeUtils
       创建对应节点并通过 update:childNode 事件挂载到链路上。
-->
<template>
...
/** 父级传入的当前子节点（新节点将挂载到其 childNode 上） */
...
/**
 * 点击菜单项：创建对应类型节点并通过事件挂载到当前节点的 childNode
 * @param {Object} item 菜单项（含 key 标识节点类型）
 */
