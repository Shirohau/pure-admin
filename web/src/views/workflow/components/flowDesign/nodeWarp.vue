<!--
  ...
  2. v-focus 指令：AntFlow 在 main.js 全局注册，本项目未注册 → 在 script setup 内定义局部 vFocus 指令
  作用：递归渲染流程节点（普通节点/条件网关/并行审批分支），支持节点名称
       行内编辑、节点删除、添加条件/并行分支、点击节点打开对应配置抽屉，
       并通过 update:node 事件将修改后的节点结构向上传递。
-->
<template>
  <div class="node-warp">
    <!-- 1. 普通节点 -->
...
import { NodeType as NodeTypeEnum, placeholderList } from '@/views/workflow/utils/workflow/constant.js';
...
/**
 * 局部 v-focus 指令（原 AntFlow 在 main.js 全局注册，本项目未注册故在组件内定义）：
 * 元素挂载后自动聚焦，用于节点名称行内编辑输入框
 */
const vFocus = {
  mounted(el) {
    el.focus();
...
/**
 * 删除节点（普通节点直接删除；网关/并行节点删除分支，分支数 <= 2 时整体删除）
